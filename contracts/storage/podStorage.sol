...
contract podStorage {
    ...
    enum YieldMechanism { AAVE, YIELD_FARMING }
    ...
    struct betInfo {
        ...
        YieldMechanism yieldMechanism; // Interest Method 
    ...
    function setBetTokens(uint256 betId, address _tokenAddress, address _interestToken) public {
        betIdTokensMapping[betId].tokenAddress = _tokenAddress;
        betIdTokensMapping[betId].interestToken = _interestToken;
    }
    
    function getBetTokens(uint256 betId) public view returns(address, address){
        return (
            betIdTokensMapping[betId].tokenAddress,
            betIdTokensMapping[betId].interestToken
        ); 
    }
